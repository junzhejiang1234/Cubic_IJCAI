...
def process_stock_data(stocks_data):
    ...
    参数:
    stocks_data: 包含多个股票数据的DataFrame，必须包含以下列：
                'symbol', 'close', 'volume', 'high', 'low', 'open'
    
    返回:
    处理后的DataFrame，包含所有标bit_predictions
    """
...
def main():
    ...
    for i in range(len(test_predictions) - window_size + 1):
        window_ic = stats.spearmanr(test_actuals[i:i+window_size], test_predictions[i:i+window_size])[0]
