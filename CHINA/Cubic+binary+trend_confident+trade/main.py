...
def load_features_data(file_path='d:/BaiduNetdiskDownload/mamba (2)/mamba/16_indicators.npz'):
...
class StockLSTM(nn.Module):
    def __init__(self, feature_dim=9, d_model=64, hidden_dim=128, num_layers=2, dropout=0.1, 
                 num_stocks=29, look_back=5, num_binary_positions=15, device='cuda'):
        super().__init__()
        ...
        input_dim = hidden_dim * num_stocks * look_back
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, self.look_back * self.num_stocks, self.feature_dim)
        ...
        predictions = logits.view(batch_size, self.num_binary_positions, 2)
        return predictions
...
def calculate_alignment(pred_float, target_float):
    if pred_float * target_float > 0:
        return 1
    elif pred_float * target_float < 0:
        return -1
    else:
        return 0
...
class ModelTrainer:
    ...
    def train_epoch(self, train_loader):
        ...
        for data, target in train_loader:
            ...
            bit_losses = []
            bit_confident_losses = []
            
            for bit_pos in range(len(output)):
                bit_predictions = output[bit_pos]
                bit_targets = target[bit_pos]
                predicted_classes = torch.argmax(bit_predictions, dim=1)
                predicted_logits = bit_predictions[torch.arange(len(bit_predictions)), predicted_classes]
                
                probabilities = F.softmax(bit_predictions, dim=1)
                predicted_probs = probabilities[torch.arange(len(probabilities)), predicted_classes]
                bit_targets_try = bit_targets.unsqueeze(0)
                bit_prediction_try = predicted_classes.unsqueeze(0) 
                bit_targets_try = bit_targets_try.cpu().numpy().tolist()
                bit_prediction_try = bit_prediction_try.cpu().numpy().tolist()
                
                log_probs = torch.log(predicted_probs[0])
                sum_log_probs = log_probs
                geometric_mean = torch.exp(sum_log_probs / len(predicted_probs)).item()
                test_target = [binary_to_float(row, min_val=-1, max_val=1, precision=0.0001) for row in bit_targets_try]
                test_predictions = [binary_to_float(row, min_val=-1, max_val=1, precision=0.0001) for row in bit_prediction_try]
                alignment = calculate_alignment(test_predictions[0], test_target[0])
                confident_loss = 1
                
                if alignment == 1:
                    confident_loss = 1 - geometric_mean
                elif alignment == -1:
                    confident_loss = 1 + geometric_mean
                else:
                    confident_loss = 1

                weights = torch.tensor([2**i for i in range(15, 0, -1)], dtype=torch.float32, device=self.device)
                weights = weights / weights.sum()

                bit_loss = self.criterion(bit_predictions, bit_targets)
                bit_loss += confident_loss
        ...
        with torch.no_grad():
            for data, target in val_loader:
                ...
                for bit_pos in range(len(output)):
                    ...
                    bit_loss += confident_loss
                    bit_losses.append(bit_loss)
                    predictions_confident.append(geometric_mean)

                all_bit_losses.append([loss.item() for loss in bit_losses])
                loss = torch.stack(bit_losses).mean()
                total_loss += loss
                predictions_confident_all.append(predictions_confident)

                preds = torch.argmax(output, dim=2)
                predictions.append(preds.cpu())
                actuals.append(target.cpu())

            predictions_confident_all = [torch.tensor(item) if not isinstance(item, torch.Tensor) else item 
                                for item in predictions_confident_all]
            predictions = torch.cat(predictions, dim=0).numpy()
            predictions_confident_all = torch.cat(predictions_confident_all, dim=0).numpy()
            ...
            return total_loss / len(val_loader), predictions, actuals, predictions_confident_all
...
def train(self, train_loader, val_loader, num_epochs=100, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        val_loss, predictions, actuals, prediction = self.validate(val_loader)
...
def backtest_result(test_predictions, test_actuals, confident):
    test_predictions = np.array(test_predictions)
    test_actuals = np.array(test_actuals)
    confident = np.array(confident)
    ...
    for day in range(len(test_predictions)):
        prediction = test_predictions[day]
        confidence = confident[day]
        actual_return = test_actuals[day]
        ...
        if prediction > 0:
            if 0.5 <= confidence < 0.7:
                ...
                holding += buy_amount
                cash -= buy_amount
            elif 0.7 <= confidence <= 1:
                holding += cash
                cash = 0

        elif prediction < 0:
            if 0.5 <= confidence < 0.7:
                sell_amount = holding / 2
                holding -= sell_amount
                cash += sell_amount
            elif 0.7 <= confidence <= 1:
                cash += holding
        ...
        holding *= (1 + actual_return)
        portfolio_value = cash + holding
        portfolio_values.append(portfolio_value)
        
        if day > 0:
            daily_return = (portfolio_values[day] - portfolio_values[day - 1]) / portfolio_values[day - 1]
            position_returns.append(daily_return)
        else:
    ...
    print(f"Annual Return (AR): {ar*100:.2f}%")
    print(f"Market Annual Return: {market_ar*100:.2f}%")
...
def main():
    ...
    CSI100_returns = df['CSI100_returns'].tolist()
    X, dates = load_features_data()
    print("X shape:", X.shape)

    Y = np.array(CSI100_returns[WINDOW_SIZE:])
    Y[-1] = 0
    ...
    test_loss, test_predictions, test_actuals, predictions_confident = trainer.validate(test_loader)
    test_actuals = np.array(test_actuals) 
    test_predictions = np.array(test_predictions)
    ...
    direction_accuracy, sharpe_ratio, ar, position_return = backtest_result(test_predictions, test_actuals, predictions_confident)
...
if __name__ == "__main__":
    try:
        ...
        print(f"ICIR - Mean: {icir_mean:.4f}, Std: {icir_std:.4f}")
        print(f"Direction Accuracy - Mean: {direction_accuracy_mean:.4f}, Std: {direction_accuracy_std:.4f}")
        print(f"Sharpe Ratio - Mean: {sharpe_ratio_mean:.4f}, Std: {sharpe_ratio_std:.4f}")
        print(f"AR - Mean: {ar_mean:.4f}, Std: {ar_std:.4f}")    
